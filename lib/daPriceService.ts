...
class DAPriceService {
  ...
  async getCurrentPrices(commodities: Commodity[]): Promise<DAPriceData[]> {
    ...
    try {
      ...
      console.log('🔄 Falling back to enhanced mock data based on DA ranges...');
      return this.getEnhancedDAMockData(commodities);
    }
  }
  ...
  private async generateGeminiForecasts(priceData: DAPriceData[]): Promise<DAForecastData[]> {
    console.log('🔄 Using seasonal forecasts (Gemini API temporarily disabled)');
    
    // Skip Gemini API calls entirely for now and use seasonal forecasts
    return priceData.map(price => this.getSeasonalForecast(price));
  }
