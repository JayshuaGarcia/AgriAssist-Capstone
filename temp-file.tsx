...
import { ActivityIndicator, Dimensions, FlatList, Modal, RefreshControl, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export default function PriceMonitoringScreen() {
  ...
  const fetchPriceData = async () => {
    ...
    try {
      ...
      if (storedPriceData.length > 0) {
        ...
        console.log('?? Sample price data:', storedPriceData.slice(0, 3));
        ...
        const updatedCommodities = COMMODITY_DATA.map(commodity => {
          ...
          if (price) {
            const updated = priceMonitoringService.updateCommodityWithPrices(commodity, price);
            console.log(`?? Updated ${commodity.name}: ${commodity.currentPrice} ? ${updated.currentPrice}`);
            return updated;
          }
          return commodity;
        });

        const withPrices = updatedCommodities.filter(c => c.currentPrice);
        console.log('?? Updated commodities with stored data:', {
          count: updatedCommodities.length,
          withPrices: withPrices.length,
          sampleWithPrices: withPrices.slice(0, 3).map(c => ({
            name: c.name,
  ...
  const filteredCommodities = React.useMemo(() => {
    ...
    // Debug: Log commodity state
    console.log('?? Filtered commodities:', {
      total: commodities.length,
      filtered: filtered.length,
      withPrices: filtered.filter(c => c.currentPrice).length,
      firstFew: filtered.slice(0, 3).map(c => ({
        name: c.name,
        currentPrice: c.currentPrice,
        hasPrice: !!c.currentPrice
      }))
    });
  ...
  const renderCommodityItem = ({ item }: { item: Commodity }) => {
    ...
    if (item.id === 'kadiwa-premium' || item.id === 'kadiwa-well-milled') {
      console.log(`?? Rendering ${item.name}: currentPrice = ${item.currentPrice}, hasPrice = ${!!item.currentPrice}`);
    }
