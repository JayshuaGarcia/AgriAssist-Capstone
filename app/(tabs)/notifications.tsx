...
const notificationsImages: { [key: string]: any } = {
  'Reminders': require('../../assets/images/Reminders.png'),
  'Seasonal Calendar': require('../../assets/images/Seasonal Calendar.png'),
  'Weather Feed': require('../../assets/images/Weather Feed.png'),
};
...
export default function NotificationsScreen() {
  ...
  const getNotificationsFeatures = () => {
    const baseFeatures = [
      {
        label: 'Seasonal Calendar',
        icon: 'calendar-month-outline',
        route: '/notifications/seasonal-calendar',
      },
      {
        label: 'Weather Feed',
        icon: 'weather-cloudy',
        route: '/notifications/weather-feed',
      },
    ];

    // Only show Reminders for BAEWs role
    if (profile.role === 'BAEWs') {
      baseFeatures.unshift({
        label: 'Reminders',
  ...
  const notificationsFeatures = getNotificationsFeatures();
  ...
  const handlePress = (item: typeof notificationsFeatures[0]) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1, marginLeft: 8, marginRight: 8 }}>
          <SearchBar
            ...
            data={notificationsFeatures.map(f => ({ id: f.route, title: f.label, icon: f.icon }))}
            onSearch={query => {
              const match = notificationsFeatures.find(f => f.label.toLowerCase() === query.toLowerCase());
              if (match) router.push(match.route);
            }}
            onSelect={item => {
              const match = notificationsFeatures.find(f => f.label === item.title);
              if (match) router.push(match.route);
            }}
          />
        </View>
      ...
      <View style={styles.categoryRow}>
        <Text style={[styles.categoryTitle, { color: '#111' }]}>Notifications & Calendar</Text>
      </View>
      {/* Notifications Grid */}
      <View style={styles.grid}>
        {notificationsFeatures.map((item, idx) => (
          <View key={idx} style={styles.catCol}>
            ...
            >
              <RNImage
                source={notificationsImages[item.label]}
                style={styles.catImg}
                resizeMode="contain"
              />
            </TouchableOpacity>
