...
import { fetchPlantingRecords } from '../../../services/plantingUploadService';

const GREEN = '#16543a';
const WHITE = '#ffffff';
const RECORDS_PER_PAGE = 20; // Load only 20 records at a time

export default function PlantingTrackerRecordsScreen() {
  const router = useRouter();
  const { profile } = useAuth();
  const { barangay } = useBarangay();
  const [allPlantingData, setAllPlantingData] = useState<any[]>([]);
  const [displayedPlanting, setDisplayedPlanting] = useState<any[]>([]);
  ...
  }, [allPlantingData, currentPage]);

  const loadRecords = async () => {
    try {
      setLoading(true);
      const records = await fetchPlantingRecords(barangay || undefined);
      ...
      setAllPlantingData(filteredRecords);
      setTotalPages(Math.ceil((filteredRecords?.length || 0) / RECORDS_PER_PAGE));
    } catch (error) {
      Alert.alert('Error', 'Failed to load planting tracker data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateDisplayedRecords = () => {
    ...
    const records = allPlantingData.slice(startIndex, endIndex);
    setDisplayedPlanting(records);
  };
  ...
  const renderEmptyComponent = () => (
    <View style={{ height: 120, justifyContent: 'center', alignItems: 'center' }}>
      <Text style={{ color: '#888', fontStyle: 'italic', fontSize: 16 }}>No planting records found.</Text>
    </View>
  );
        ...
        <View style={styles.headerTextCol}>
          ...
          <Text style={styles.headerSubtitle}>
            {profile.role === 'Admin' && barangay 
              ...
              : 'View all planting tracker records'
            }
          </Text>
        </View>
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <>
            ...
            {/* Pagination Controls */}
            {allPlantingData.length > RECORDS_PER_PAGE && (
              <View style={styles.paginationContainer}>
                <Text style={styles.paginationInfo}>
                  Showing {((currentPage - 1) * RECORDS_PER_PAGE) + 1} - {Math.min(currentPage * RECORDS_PER_PAGE, allPlantingData.length)} of {allPlantingData.length} records
                </Text>
