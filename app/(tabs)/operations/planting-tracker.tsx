...
import { deleteAllPlantingRecords, deletePlantingRecord, fetchPlantingRecords, updatePlantingRecord, uploadPlantingRecord } from '../../../services/plantingUploadService';
...
export default function PlantingTrackerScreen() {
  ...
  const [groupedPlantingRecords, setGroupedPlantingRecords] = useState<{ [year: string]: any[] }>({});
  ...
  React.useEffect(() => {
    if (profile.role === 'Viewer') {
      ...
      setRecordType('planting-tracker');
  ...
  const handleSave = async () => {
    ...
    if (parseFloat(total) === 0) {
      Alert.alert('Required Fields', 'Please enter area planted for at least one week');
      return;
    }
    ...
    try {
      if (editingId) {
        await updatePlantingRecord(editingId, record);
        Alert.alert('Success', 'Planting tracker updated!');
        setEditingId(null);
      } else {
        await uploadPlantingRecord(record);
        Alert.alert('Success', 'Planting tracker saved and uploaded!');
      }
      handleClear();
    } catch (error) {
  ...
  const handleDeleteAllRecords = async () => {
    try {
      await deleteAllPlantingRecords();
  ...
  const handleViewRecords = async () => {
    try {
      ...
      setGroupedPlantingRecords(grouped);
      ...
      if (result.assets && result.assets.length > 0) {
        ...
        reader.onload = async (e) => {
          ...
          for (const record of json) {
            ...
            await uploadPlantingRecord(sanitized);
          }
          setUploading(false);
  ...
  const getMonthsForYear = (year: string) => {
    const records = groupedPlantingRecords[year] || [];
  ...
  const allYears = Array.from(new Set(["2025", ...Object.keys(groupedPlantingRecords)])).sort((a, b) => b.localeCompare(a));
    ...
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.headerTextCol}>
          ...
          <Text style={styles.headerSubtitle}>Track planting activities and progress.</Text>
        </View>
      ...
      {profile.role !== 'Viewer' ? (
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          <View style={styles.formContainer}>
            ...
            {/* Weekly Planting Data Section */}
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <MaterialCommunityIcons name="calendar-week" size={24} color={GREEN} />
                <Text style={styles.sectionTitle}>Weekly Planting Data</Text>
              </View>
      ...
      {/* Modal for Planting.xlsx records remains for non-Viewers */}
      <Modal visible={modalVisible} animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={{ flex: 1, backgroundColor: '#fff', padding: 20, alignItems: 'center', justifyContent: 'flex-start' }}>
          {/* Title on top */}
          <Text style={{ fontSize: 22, fontWeight: 'bold', marginBottom: 10, alignSelf: 'center' }}>Planting Records</Text>
