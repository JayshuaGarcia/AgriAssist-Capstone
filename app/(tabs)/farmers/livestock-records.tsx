...
import { fetchLivestockRecords } from '../../../services/livestockUploadService';
...
export default function LivestockRecordsScreen() {
  ...
  const [allLivestockData, setAllLivestockData] = useState<any[]>([]);
  const [displayedLivestock, setDisplayedLivestock] = useState<any[]>([]);
  ...
  }, [allLivestockData, currentPage]);

  const loadRecords = async () => {
    try {
      setLoading(true);
      const records = await fetchLivestockRecords(barangay || undefined);
      ...
      setAllLivestockData(filteredRecords);
  ...
  const updateDisplayedRecords = () => {
    ...
    const records = allLivestockData.slice(startIndex, endIndex);
    setDisplayedLivestock(records);
  };
    ...
    return (
      ...
      >
        ...
        <View style={[styles.tableCell, { width: 100 }]}>
          <Text style={styles.tableCellText}>{item.maleCount}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.femaleCount}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{parseInt(item.maleCount || '0') + parseInt(item.femaleCount || '0')}</Text>
        </View>
      </View>
    );
  };
  ...
  const renderEmptyComponent = () => (
    <View style={{ height: 120, justifyContent: 'center', alignItems: 'center' }}>
      <Text style={{ color: '#888', fontStyle: 'italic', fontSize: 16 }}>No livestock records found.</Text>
    </View>
  );
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <>
            <ScrollView horizontal style={{ flexGrow: 0 }} contentContainerStyle={{ flexGrow: 1 }}>
              <View style={styles.tableContainer}>
                ...
                <View style={styles.tableHeaderRow}>
                  {['No.', 'Farmer Name', 'Type of Animal', 'Male Count', 'Female Count', 'Total'].map((header, hIdx) => (
                    <View
                      key={hIdx}
                      style={[
                        styles.tableHeaderCell,
                        { width: [60, 120, 120, 100, 120, 120][hIdx] },
                      ]}
                    >
            ...
            {/* Pagination Controls */}
            {allLivestockData.length > RECORDS_PER_PAGE && (
              <View style={styles.paginationContainer}>
                <Text style={styles.paginationInfo}>
                  Showing {((currentPage - 1) * RECORDS_PER_PAGE) + 1} - {Math.min(currentPage * RECORDS_PER_PAGE, allLivestockData.length)} of {allLivestockData.length} records
                </Text>
