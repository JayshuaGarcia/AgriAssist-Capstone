...
import { fetchFarmerProfiles } from '../../../services/farmerProfileUploadService';
...
export default function FarmerProfilesRecordsScreen() {
  const router = useRouter();
  const { profile } = useAuth();
  const { barangay } = useBarangay();
  const [allFarmerData, setAllFarmerData] = useState<any[]>([]);
  const [displayedFarmer, setDisplayedFarmer] = useState<any[]>([]);
  ...
  }, [allFarmerData, currentPage]);

  const loadRecords = async () => {
    try {
      setLoading(true);
      const records = await fetchFarmerProfiles(barangay || undefined);
      ...
      setAllFarmerData(filteredRecords);
      setTotalPages(Math.ceil((filteredRecords?.length || 0) / RECORDS_PER_PAGE));
    } catch (error) {
      Alert.alert('Error', 'Failed to load farmer profiles data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateDisplayedRecords = () => {
    ...
    const records = allFarmerData.slice(startIndex, endIndex);
    setDisplayedFarmer(records);
  };
  ...
  const renderTableRow = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      ...
      >
        ...
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{formatBirthday(item.birthday)}</Text>
        </View>
        <View style={[styles.tableCell, { width: 80 }]}>
          <Text style={styles.tableCellText}>{item.sex}</Text>
        </View>
        <View style={[styles.tableCell, { width: 140 }]}>
          <Text style={styles.tableCellText}>{item.contactNumber}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.cropName}</Text>
        </View>
        <View style={[styles.tableCell, { width: 100 }]}>
          <Text style={styles.tableCellText}>{item.cropArea}</Text>
        </View>
      </View>
    );
  };
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <>
                  ...
                  {['No.', 'First Name', 'Middle Name', 'Last Name', 'Birthday', 'Sex', 'Contact Number', 'Crop Name', 'Crop Area'].map((header, hIdx) => (
                    <View
                      ...
                      style={[
                        ...
                        { width: [60, 120, 120, 120, 120, 80, 140, 120, 100][hIdx] },
                      ]}
                    >
            ...
            {/* Pagination Controls */}
            {allFarmerData.length > RECORDS_PER_PAGE && (
              <View style={styles.paginationContainer}>
                <Text style={styles.paginationInfo}>
                  Showing {((currentPage - 1) * RECORDS_PER_PAGE) + 1} - {Math.min(currentPage * RECORDS_PER_PAGE, allFarmerData.length)} of {allFarmerData.length} records
                </Text>
