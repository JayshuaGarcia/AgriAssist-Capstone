...
const operationsImages: { [key: string]: any } = {
  'Planting Tracker': require('../../assets/images/Planting Tracker.png'),
  'Harvest Tracker': require('../../assets/images/Harvest Tracker.png'),
  'Training Attendance': require('../../assets/images/Training Attendance.png'),
  'Photo Capture with GPS': require('../../assets/images/Photo Capture with GPS.png'),
};
...
export default function OperationsScreen() {
  ...
  const getOperationsFeatures = () => {
    const baseFeatures = [
      {
        label: 'Planting Tracker',
        icon: 'tractor',
        route: '/operations/planting-tracker',
      },
      {
        label: 'Harvest Tracker',
        icon: 'food-apple',
        route: '/operations/harvest-tracker',
      },
    ];

    // Only show Training Attendance and Photo Capture for BAEWs role
    if (profile.role === 'BAEWs') {
      baseFeatures.push(
        {
          label: 'Training Attendance',
          icon: 'account-group-outline',
          route: '/operations/training-attendance',
        },
        {
          label: 'Photo Capture with GPS',
          icon: 'camera-outline',
          route: '/operations/photo-gps',
        }
      );
    }
  ...
  const operationsFeatures = getOperationsFeatures();
  ...
  const handlePress = (item: typeof operationsFeatures[0]) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1, marginLeft: 8, marginRight: 8 }}>
          <SearchBar
            ...
            data={operationsFeatures.map(f => ({ id: f.route, title: f.label, icon: f.icon }))}
            onSearch={query => {
              const match = operationsFeatures.find(f => f.label.toLowerCase() === query.toLowerCase());
              if (match) router.push(match.route);
            }}
            onSelect={item => {
              const match = operationsFeatures.find(f => f.label === item.title);
              if (match) router.push(match.route);
            }}
          />
        </View>
      ...
      <View style={styles.categoryRow}>
        <Text style={[styles.categoryTitle, { color: '#111' }]}>Operations</Text>
      </View>
      {/* Operations Grid */}
      <View style={styles.grid}>
        {operationsFeatures.map((item, idx) => (
          <View key={idx} style={styles.catCol}>
            ...
            >
              <RNImage
                source={operationsImages[item.label]}
                style={styles.catImg}
                resizeMode="contain"
              />
            </TouchableOpacity>
