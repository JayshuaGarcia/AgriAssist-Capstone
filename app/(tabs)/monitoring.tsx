...
const monitoringFeatures = [
  {
    label: 'Fertilizer/Pesticide Logs',
    icon: 'spray',
    route: '/monitoring/fertilizer-logs',
  },
  {
    label: 'Crop Monitoring',
    icon: 'leaf-circle',
    route: '/monitoring/crop-monitoring',
  },
  {
    label: 'Barangay Commodity Map',
    icon: 'map-marker-distance',
    route: '/monitoring/commodity-map',
  },
  {
    label: 'Accomplishment Reports',
    icon: 'clipboard-list-outline',
    route: '/monitoring/accomplishment-reports',
  },
];
...
const monitoringImages: { [key: string]: any } = {
  'Fertilizer/Pesticide Logs': require('../../assets/images/Fertilizer_Pesticide Logs.png'),
  'Crop Monitoring': require('../../assets/images/Crop Monitoring.png'),
  'Barangay Commodity Map': require('../../assets/images/Barangay Commodity Map.png'),
  'Accomplishment Reports': require('../../assets/images/Accomplishment Reports.png'),
};
...
export default function MonitoringScreen() {
  ...
  const handlePress = (item: typeof monitoringFeatures[0]) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1, marginLeft: 8, marginRight: 8 }}>
          <SearchBar
            ...
            data={monitoringFeatures.map(f => ({ id: f.route, title: f.label, icon: f.icon }))}
            onSearch={query => {
              const match = monitoringFeatures.find(f => f.label.toLowerCase() === query.toLowerCase());
              if (match) router.push(match.route);
            }}
            onSelect={item => {
              const match = monitoringFeatures.find(f => f.label === item.title);
              if (match) router.push(match.route);
            }}
          />
        </View>
      ...
      <View style={styles.categoryRow}>
        <Text style={[styles.categoryTitle, { color: '#111' }]}>Monitoring</Text>
      </View>
      {/* Monitoring Grid */}
      <View style={styles.grid}>
        {monitoringFeatures.map((item, idx) => (
          <View key={idx} style={styles.catCol}>
            ...
            >
              <RNImage
                source={monitoringImages[item.label]}
                style={styles.catImg}
                resizeMode="contain"
              />
            </TouchableOpacity>
