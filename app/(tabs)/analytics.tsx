...
const analyticsFeatures = [
  {
    label: 'Crop Trend Analytics',
    icon: 'chart-line-variant',
    route: '/analytics/crop-trend',
  },
  {
    label: 'Forecast Tools',
    icon: 'calendar-clock',
    route: '/analytics/forecast-tools',
  },
  {
    label: 'Farmer Performance Insight',
    icon: 'account-cog',
  ...
  },
  {
    ...
    route: '/analytics/comparative-analysis',
  },
];
...
const analyticsImages: { [key: string]: any } = {
  'Crop Trend Analytics': require('../../assets/images/Crop Trend Analytics.png'),
  'Forecast Tools': require('../../assets/images/Forecast Tools.png'),
  'Farmer Performance Insight': require('../../assets/images/Farmer Performance Insight.png'),
  'Comparative Analysis': require('../../assets/images/Comparative Analysis.png'),
};
...
export default function AnalyticsScreen() {
  ...
  const handlePress = (item: typeof analyticsFeatures[0]) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1, marginLeft: 8, marginRight: 8 }}>
          <SearchBar
            ...
            data={analyticsFeatures.map(f => ({ id: f.route, title: f.label, icon: f.icon }))}
            onSearch={query => {
              const match = analyticsFeatures.find(f => f.label.toLowerCase() === query.toLowerCase());
              if (match) router.push(match.route);
            }}
            onSelect={item => {
              const match = analyticsFeatures.find(f => f.label === item.title);
              if (match) router.push(match.route);
            }}
          />
        </View>
      ...
      <View style={styles.categoryRow}>
        <Text style={[styles.categoryTitle, { color: '#111' }]}>Analytics & Forecasting</Text>
      </View>
      {/* Analytics Grid */}
      <View style={styles.grid}>
        {analyticsFeatures.map((item, idx) => (
          <View key={idx} style={styles.catCol}>
            ...
            >
              <RNImage
                source={analyticsImages[item.label]}
                style={styles.catImg}
                resizeMode="contain"
              />
            </TouchableOpacity>
