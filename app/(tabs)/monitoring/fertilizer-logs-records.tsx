...
import { fetchFertilizerLogs } from '../../../services/fertilizerLogService';

const GREEN = '#16543a';
const WHITE = '#ffffff';
const RECORDS_PER_PAGE = 20; // Load only 20 records at a time

export default function FertilizerLogsRecordsScreen() {
  const router = useRouter();
  const { profile } = useAuth();
  const { barangay } = useBarangay();
  const [allFertilizerData, setAllFertilizerData] = useState<any[]>([]);
  const [displayedFertilizer, setDisplayedFertilizer] = useState<any[]>([]);
  ...
  }, [allFertilizerData, currentPage]);

  const loadRecords = async () => {
    try {
      setLoading(true);
      const records = await fetchFertilizerLogs(barangay || undefined);
      ...
      setAllFertilizerData(filteredRecords);
      setTotalPages(Math.ceil((filteredRecords?.length || 0) / RECORDS_PER_PAGE));
    } catch (error) {
      Alert.alert('Error', 'Failed to load fertilizer logs data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateDisplayedRecords = () => {
    ...
    const records = allFertilizerData.slice(startIndex, endIndex);
    setDisplayedFertilizer(records);
  };
  ...
  const renderTableRow = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      ...
      >
        ...
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.birthday}</Text>
        </View>
        <View style={[styles.tableCell, { width: 100 }]}>
          <Text style={styles.tableCellText}>{item.suffix}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.voucherValue}</Text>
        </View>
      </View>
    );
  };
  ...
  const renderEmptyComponent = () => (
    <View style={{ height: 120, justifyContent: 'center', alignItems: 'center' }}>
      <Text style={{ color: '#888', fontStyle: 'italic', fontSize: 16 }}>No fertilizer logs found.</Text>
    </View>
  );
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <>
            <ScrollView horizontal style={{ flexGrow: 0 }} contentContainerStyle={{ flexGrow: 1 }}>
              <View style={styles.tableContainer}>
                ...
                <View style={styles.tableHeaderRow}>
                  {['No.', 'First Name', 'Last Name', 'Contact Number', 'Birthday', 'Suffix', 'Voucher Value'].map((header, hIdx) => (
                    <View
                      key={hIdx}
                      style={[
                        styles.tableHeaderCell,
                        { width: [60, 120, 120, 140, 120, 100, 120][hIdx] },
                      ]}
                    >
            ...
            {/* Pagination Controls */}
            {allFertilizerData.length > RECORDS_PER_PAGE && (
              <View style={styles.paginationContainer}>
                <Text style={styles.paginationInfo}>
                  Showing {((currentPage - 1) * RECORDS_PER_PAGE) + 1} - {Math.min(currentPage * RECORDS_PER_PAGE, allFertilizerData.length)} of {allFertilizerData.length} records
                </Text>
