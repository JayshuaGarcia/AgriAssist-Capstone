...
import { fetchAccomplishmentReports } from '../../../services/accomplishmentReportService';

const GREEN = '#16543a';
const WHITE = '#ffffff';
const RECORDS_PER_PAGE = 20; // Load only 20 records at a time

export default function AccomplishmentReportsRecordsScreen() {
  const router = useRouter();
  const { profile } = useAuth();
  const { barangay } = useBarangay();
  const [allAccomplishmentData, setAllAccomplishmentData] = useState<any[]>([]);
  const [displayedAccomplishment, setDisplayedAccomplishment] = useState<any[]>([]);
  ...
  }, [allAccomplishmentData, currentPage]);

  const loadRecords = async () => {
    try {
      setLoading(true);
      const records = await fetchAccomplishmentReports(barangay || undefined);
      ...
      setAllAccomplishmentData(filteredRecords);
      setTotalPages(Math.ceil((filteredRecords?.length || 0) / RECORDS_PER_PAGE));
    } catch (error) {
      Alert.alert('Error', 'Failed to load accomplishment reports data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateDisplayedRecords = () => {
    ...
    const records = allAccomplishmentData.slice(startIndex, endIndex);
    setDisplayedAccomplishment(records);
  };
    ...
    return (
      ...
      >
        ...
        <View style={[styles.tableCell, { width: 400 }]}>
          <Text style={styles.tableCellText}>{item.accomplishment}</Text>
        </View>
        <View style={[styles.tableCell, { width: 200 }]}>
          <Text style={styles.tableCellText}>{item.remarks}</Text>
        </View>
      </View>
    );
  };
  ...
  const renderEmptyComponent = () => (
    <View style={{ height: 120, justifyContent: 'center', alignItems: 'center' }}>
      <Text style={{ color: '#888', fontStyle: 'italic', fontSize: 16 }}>No accomplishment reports found.</Text>
    </View>
  );
        ...
        <View style={styles.headerTextCol}>
          ...
          <Text style={styles.headerSubtitle}>
            {profile.role === 'Admin' && barangay 
              ...
              : 'View all accomplishment reports'
            }
          </Text>
        </View>
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <>
            <ScrollView horizontal style={{ flexGrow: 0 }} contentContainerStyle={{ flexGrow: 1 }}>
              <View style={styles.tableContainer}>
                ...
                <View style={styles.tableHeaderRow}>
                  {['No.', 'Date', 'Accomplishment', 'Remarks'].map((header, hIdx) => (
                    <View
                      key={hIdx}
                      style={[
                        styles.tableHeaderCell,
                        { width: [60, 120, 400, 200][hIdx] },
                      ]}
                    >
            ...
            {/* Pagination Controls */}
            {allAccomplishmentData.length > RECORDS_PER_PAGE && (
              <View style={styles.paginationContainer}>
                <Text style={styles.paginationInfo}>
                  Showing {((currentPage - 1) * RECORDS_PER_PAGE) + 1} - {Math.min(currentPage * RECORDS_PER_PAGE, allAccomplishmentData.length)} of {allAccomplishmentData.length} records
                </Text>
