...
import { fetchCropMonitoringReports } from '../../../services/cropMonitoringService';

const GREEN = '#16543a';
const WHITE = '#ffffff';
const RECORDS_PER_PAGE = 20; // Load only 20 records at a time

export default function CropMonitoringRecordsScreen() {
  const router = useRouter();
  const { profile } = useAuth();
  const { barangay } = useBarangay();
  const [allCropData, setAllCropData] = useState<any[]>([]);
  const [displayedCrop, setDisplayedCrop] = useState<any[]>([]);
  ...
  }, [allCropData, currentPage]);

  const loadRecords = async () => {
    try {
      setLoading(true);
      const records = await fetchCropMonitoringReports(barangay || undefined);
      ...
      setAllCropData(filteredRecords);
      setTotalPages(Math.ceil((filteredRecords?.length || 0) / RECORDS_PER_PAGE));
    } catch (error) {
      Alert.alert('Error', 'Failed to load crop monitoring data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateDisplayedRecords = () => {
    ...
    const records = allCropData.slice(startIndex, endIndex);
    setDisplayedCrop(records);
  };
    ...
    return (
      ...
      >
        ...
        <View style={[styles.tableCell, { width: 140 }]}>
          <Text style={styles.tableCellText}>{item.farmerName}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.cropType}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.monitoringDate}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.cropCondition}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.fieldLocation}</Text>
        </View>
        <View style={[styles.tableCell, { width: 120 }]}>
          <Text style={styles.tableCellText}>{item.growthStage}</Text>
        </View>
        <View style={[styles.tableCell, { width: 100 }]}>
          <Text style={styles.tableCellText}>{item.pestSightings}</Text>
        </View>
        <View style={[styles.tableCell, { width: 100 }]}>
          <Text style={styles.tableCellText}>{item.diseaseObservations}</Text>
        </View>
        <View style={[styles.tableCell, { width: 100 }]}>
          <Text style={styles.tableCellText}>{item.weatherConditions}</Text>
        </View>
        <View style={[styles.tableCell, { width: 100 }]}>
          <Text style={styles.tableCellText}>{item.irrigationStatus}</Text>
        </View>
        <View style={[styles.tableCell, { width: 180 }]}>
          <Text style={styles.tableCellText}>{item.additionalNotes}</Text>
        </View>
      </View>
    );
  };
  ...
  const renderEmptyComponent = () => (
    <View style={{ height: 120, justifyContent: 'center', alignItems: 'center' }}>
      <Text style={{ color: '#888', fontStyle: 'italic', fontSize: 16 }}>No crop monitoring records found.</Text>
    </View>
  );
        ...
        <View style={styles.headerTextCol}>
          ...
          <Text style={styles.headerSubtitle}>
            {profile.role === 'Admin' && barangay 
              ...
              : 'View all crop monitoring records'
            }
          </Text>
        </View>
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <>
            <ScrollView horizontal style={{ flexGrow: 0 }} contentContainerStyle={{ flexGrow: 1 }}>
              <View style={styles.tableContainer}>
                ...
                <View style={styles.tableHeaderRow}>
                  {['No.', 'Farmer Name', 'Crop Type', 'Date', 'Condition', 'Location', 'Growth Stage', 'Pests', 'Diseases', 'Weather', 'Irrigation', 'Notes'].map((header, hIdx) => (
                    <View
                      key={hIdx}
                      style={[
                        styles.tableHeaderCell,
                        { width: [60, 140, 120, 120, 120, 120, 120, 100, 100, 100, 100, 180][hIdx] },
                      ]}
                    >
            ...
            {/* Pagination Controls */}
            {allCropData.length > RECORDS_PER_PAGE && (
              <View style={styles.paginationContainer}>
                <Text style={styles.paginationInfo}>
                  Showing {((currentPage - 1) * RECORDS_PER_PAGE) + 1} - {Math.min(currentPage * RECORDS_PER_PAGE, allCropData.length)} of {allCropData.length} records
                </Text>
