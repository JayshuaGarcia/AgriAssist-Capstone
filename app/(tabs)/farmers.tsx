...
const farmersImages: { [key: string]: any } = {
  'Farmer Profiles': require('../../assets/images/Farmer Profiles.png'),
  'Demographics': require('../../assets/images/Demographics.png'),
  'Livestock Inventory': require('../../assets/images/Livestock Inventory.png'),
  'Duplicate Checker': require('../../assets/images/Duplicate Checker.png'),
};
...
export default function FarmersScreen() {
  ...
  const getFarmersFeatures = () => {
    const baseFeatures = [
      ...
      },
      {
        ...
        route: '/farmers/demographics',
      },
      {
        label: 'Livestock Inventory',
        icon: 'cow',
        route: '/farmers/livestock',
      },
    ];

    // Only show Duplicate Checker for BAEWs role
    if (profile.role === 'BAEWs') {
      baseFeatures.push({
        label: 'Duplicate Checker',
        icon: 'account-search-outline',
        route: '/farmers/duplicate-checker',
      });
    }
  ...
  const farmersFeatures = getFarmersFeatures();
  ...
  const handlePress = (item: typeof farmersFeatures[0]) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1, marginLeft: 8, marginRight: 8 }}>
          <SearchBar
            ...
            data={farmersFeatures.map(f => ({ id: f.route, title: f.label, icon: f.icon }))}
            onSearch={query => {
              const match = farmersFeatures.find(f => f.label.toLowerCase() === query.toLowerCase());
              if (match) router.push(match.route);
            }}
            onSelect={item => {
              const match = farmersFeatures.find(f => f.label === item.title);
              if (match) router.push(match.route);
            }}
          />
        </View>
      ...
      <View style={styles.categoryRow}>
        <Text style={[styles.categoryTitle, { color: '#111' }]}>Farmers</Text>
      </View>
      {/* Farmers Grid */}
      <View style={styles.grid}>
        {farmersFeatures.map((item, idx) => (
          <View key={idx} style={styles.catCol}>
            ...
            >
              <RNImage
                source={farmersImages[item.label]}
                style={styles.catImg}
                resizeMode="contain"
              />
            </TouchableOpacity>
